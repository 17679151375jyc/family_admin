...
export default {
  ...
  computed: {
    statusList: function() {
      let statusList = this.$config.community.apply.status;
        ...
        feedback: [
          {
            validator: (rule, value, callback, source, options) =>{
              let err = [];
              if (
                that.form.audit === that.statusList[2].value &&
                that.form &&
                !value
              ) {
                err = "请选择不通过的原因";
    ...
    feedbackIsShow: function() {
      if (this.form.audit === this.statusList[2].value) {
        // 如果状态改为审核不通过，显示反馈内容
        return true;
      } else {
        return false;
