  ...
  <Modal title="添加" v-model.trim="isShow" :mask-closable="false" :loading="loading" :closable="false">
    ...
    <Form ref="form" :model="form" :rules="rules" :label-width="100">
      ...
      <FormItem prop="title" label="标题">
        <Input v-model.trim="form.title" placeholder="请输入标题" style="width: 250px;" />
      </FormItem>
...
<script>
import { addMaintenance } from "@/api/messageManage";
      ...
      this.$refs["form"].validate(async valid => {
        if (valid) {
          ...
          addMaintenance({
            ...
            title,
            content,
            orderTime: new Date(orderTime).getTime() / 1000
          })
            .then(({ errorCode }) => {
              if (errorCode === 0) {
